...
class AstrophysicalSciencesCloudScraper:
    def __init__(self):
        self.scraper = cloudscraper.create_scraper()
        self.base_url = "https://web.astro.princeton.edu"
        self.events_url = f"{self.base_url}/events"
        
    def scrape_astrophysical_sciences_events(self) -> List[Dict[str, Any]]:
        """Scrape events from the Astrophysical Sciences department"""
        events = []
        
        try:
            print(f"Scraping Astrophysical Sciences events from: {self.events_url}")
            response = self.scraper.get(self.events_url)
            response.raise_for_status()
            
            soup = BeautifulSoup(response.content, 'html.parser')
            
            # Look for astrophysics event patterns
            astro_keywords = ['seminar', 'lecture', 'colloquium', 'symposium', 'workshop', 'conference', 'talk', 'astronomy', 'astrophysics']
            ...
            # Also look for any links that contain astrophysics event information
            all_links = soup.find_all('a', href=True)
            for link in all_links:
                link_text = link.get_text(strip=True)
                if (len(link_text) > 20 and 
                    any(keyword in link_text.lower() for keyword in astro_keywords) and
            ...
            print(f"Found {len(unique_events)} Astrophysical Sciences events")
            return unique_events
            
        except Exception as e:
            print(f"Error scraping Astrophysical Sciences events: {e}")
            return []
    ...
    def _extract_event_from_container(self, container) -> Dict[str, Any]:
        ...
        location_patterns = [
            r'\b(?:Peyton Hall|Ivy Lane|Room \d+)\b',
...
if __name__ == "__main__":
    scraper = AstrophysicalSciencesCloudScraper()
    events = scraper.scrape_astrophysical_sciences_events()
    print(f"Found {len(events)} Astrophysical Sciences events")
