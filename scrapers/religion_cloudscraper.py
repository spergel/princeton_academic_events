...
class ReligionCloudScraper:
    def __init__(self):
        self.scraper = cloudscraper.create_scraper()
        self.base_url = "https://religion.princeton.edu"
        self.events_url = f"{self.base_url}/events"
        
    def scrape_religion_events(self) -> List[Dict[str, Any]]:
        """Scrape events from the Religion department"""
        events = []
        
        try:
            print(f"Scraping Religion events from: {self.events_url}")
            response = self.scraper.get(self.events_url)
            response.raise_for_status()
            
            soup = BeautifulSoup(response.content, 'html.parser')
            
            # Look for event containers - Religion typically uses structured event listings
            event_containers = soup.find_all(['div', 'article'], class_=re.compile(r'event|lecture|talk|seminar|colloquium', re.I))
            ...
            if not event_containers:
                ...
                event_containers = [div for div in event_containers if any(keyword in div.get_text().lower() 
                                                                       for keyword in ['lecture', 'talk', 'seminar', 'event', 'religion', 'theology', 'colloquium'])]
            ...
            print(f"Found {len(events)} Religion events")
            return events
            
        except Exception as e:
            print(f"Error scraping Religion events: {e}")
            return []
    ...
    def _parse_page_for_events(self, soup) -> List[Dict[str, Any]]:
        ...
        for event_text in potential_events:
            ...
            if any(keyword in event_text.lower() for keyword in ['lecture', 'talk', 'seminar', 'event', 'religion', 'theology', 'colloquium']):
                event = {
...
if __name__ == "__main__":
    scraper = ReligionCloudScraper()
    events = scraper.scrape_religion_events()
    
    # Save to file
    with open('religion_events.json', 'w', encoding='utf-8') as f:
        json.dump(events, f, indent=2, ensure_ascii=False)
    
    print(f"Scraped {len(events)} events from Religion")
