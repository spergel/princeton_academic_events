...
class EnglishCloudScraper:
    def __init__(self):
        ...
        self.base_url = "https://english.princeton.edu/events"
        
    def scrape_events(self):
        """Scrape events from English department"""
        print("📖 Scraping English Department Events...")
        ...
        try:
            ...
            with open('english_cloudscraper_content.html', 'w', encoding='utf-8') as f:
                f.write(soup.prettify())
            
            events = []
            
            # Look for event containers - English typically uses different selectors
            event_selectors = [
                '.event', '.events', '.calendar-event', '.seminar',
                '.lecture', '.colloquium', '.talk', '.workshop',
                '[class*="event"]', '[class*="seminar"]', '[class*="lecture"]',
                'article', '.node', '.content-item', '.event-item'
            ]
            ...
            print(f"\n📊 English Events Summary:")
            print(f"Total events found: {len(events)}")
            
            return events
            
        except Exception as e:
            print(f"❌ Error scraping English events: {str(e)}")
            return []
    ...
    def parse_event_element(self, element, index):
        ...
        try:
            ...
            # Create event object
            event = {
                'id': f"english_{index}_{self.sanitize_id(title)}",
                'title': title,
                ...
                'source_name': 'English Department Events',
                'tags': tags,
    ...
    def extract_event_type(self, element, title):
        ...
        if any(word in title_lower for word in ['seminar', 'seminars']):
            ...
            return 'Conference'
        elif any(word in title_lower for word in ['reading', 'readings']):
            return 'Reading'
        else:
    ...
    def extract_tags(self, element, title, description):
        ...
        # English-specific tags
        english_tags = [
            'poetry', 'fiction', 'drama', 'prose', 'literature',
            'medieval', 'renaissance', 'early modern', 'modern',
            'contemporary', 'american literature', 'british literature',
            'world literature', 'comparative literature', 'creative writing',
            'literary theory', 'criticism', 'narrative', 'poetics',
            'shakespeare', 'romanticism', 'victorian', 'modernism',
            'postmodernism', 'colonial', 'postcolonial'
        ]
    ...
    def save_events(self, events, filename='english_cloudscraper_events.json'):
        ...
        output = {
            'metadata': {
                ...
                'source_url': self.base_url,
                'source': 'English CloudScraper',
                'note': 'Scraped from english.princeton.edu/events using cloudscraper'
            },
            'events': events
        }
        ...
        print(f"💾 Saved {len(events)} English events to {filename}")

if __name__ == "__main__":
    scraper = EnglishCloudScraper()
