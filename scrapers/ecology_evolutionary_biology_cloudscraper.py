...
class EcologyEvolutionaryBiologyCloudScraper:
    def __init__(self):
        self.scraper = cloudscraper.create_scraper()
        self.base_url = "https://eeb.princeton.edu"
        self.events_url = f"{self.base_url}/events"
        
    def scrape_ecology_evolutionary_biology_events(self) -> List[Dict[str, Any]]:
        """Scrape events from the Ecology and Evolutionary Biology department"""
        events = []
        
        try:
            print(f"Scraping Ecology and Evolutionary Biology events from: {self.events_url}")
            response = self.scraper.get(self.events_url)
            response.raise_for_status()
            
            soup = BeautifulSoup(response.content, 'html.parser')
            
            # Look for event containers - EEB typically uses structured event listings
            ...
            print(f"Found {len(events)} Ecology and Evolutionary Biology events")
            return events
            
        except Exception as e:
            print(f"Error scraping Ecology and Evolutionary Biology events: {e}")
            return []
...
if __name__ == "__main__":
    scraper = EcologyEvolutionaryBiologyCloudScraper()
    events = scraper.scrape_ecology_evolutionary_biology_events()
    
    # Save to file
    with open('ecology_evolutionary_biology_events.json', 'w', encoding='utf-8') as f:
        json.dump(events, f, indent=2, ensure_ascii=False)
    
    print(f"Scraped {len(events)} events from Ecology and Evolutionary Biology")
