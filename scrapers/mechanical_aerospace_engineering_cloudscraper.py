...
class MechanicalAerospaceEngineeringCloudScraper:
    def __init__(self):
        self.scraper = cloudscraper.create_scraper()
        self.base_url = "https://mae.princeton.edu"
        self.events_url = self.base_url
        
    def scrape_mechanical_aerospace_engineering_events(self) -> List[Dict[str, Any]]:
        """Scrape events from the Mechanical and Aerospace Engineering department"""
        events = []
        
        try:
            print(f"Scraping Mechanical and Aerospace Engineering events from: {self.events_url}")
            response = self.scraper.get(self.events_url)
            response.raise_for_status()
            
            soup = BeautifulSoup(response.content, 'html.parser')
            
            # Look for engineering event patterns
            eng_keywords = ['seminar', 'lecture', 'colloquium', 'symposium', 'workshop', 'conference', 'talk']
            ...
            # Also look for any links that contain engineering event information
            all_links = soup.find_all('a', href=True)
            for link in all_links:
                link_text = link.get_text(strip=True)
                if (len(link_text) > 20 and 
                    any(keyword in link_text.lower() for keyword in eng_keywords) and
            ...
            print(f"Found {len(unique_events)} Mechanical and Aerospace Engineering events")
            return unique_events
            
        except Exception as e:
            print(f"Error scraping Mechanical and Aerospace Engineering events: {e}")
            return []
    ...
    def _extract_event_from_container(self, container) -> Dict[str, Any]:
        ...
        location_patterns = [
            r'\b(?:D-219|D-220|D-221|Room \d+)\b',
...
if __name__ == "__main__":
    scraper = MechanicalAerospaceEngineeringCloudScraper()
    events = scraper.scrape_mechanical_aerospace_engineering_events()
    print(f"Found {len(events)} Mechanical and Aerospace Engineering events")
