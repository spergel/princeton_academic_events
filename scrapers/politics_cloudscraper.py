...
class PoliticsCloudScraper:
	...
	def scrape_politics_events(self):
		print("SCRAPING POLITICS EVENTS")
		print("="*60)
		try:
			with open('politics_cloudscraper_content.html', 'r', encoding='utf-8') as f:
				html = f.read()
		...
		url = 'https://politics.princeton.edu/events'
		try:
	...
	def parse_item(self, it):
		...
		if link:
			href = link['href']
			if href.startswith('/'):
				href = f"https://politics.princeton.edu{href}"
			elif not href.startswith('http'):
				href = f"https://politics.princeton.edu/{href}"
		return {
			'id': f"politics_{start_date}_{title[:20].replace(' ','_')}",
			'title': title,
			'description': title,
	...
	def extract_tags(self, title):
		...
		for tag in ['seminar','lecture','talk','colloquium','workshop','conference','symposium','politics','policy','international relations','comparative','political theory']:
	...
	def save_events(self, events):
		...
		fn='politics_cloudscraper_events.json'
		with open(fn,'w',encoding='utf-8') as f:
			json.dump({'metadata':{'total_events':len(events),'department':'Politics','scraped_at':datetime.now().isoformat()},'events':events},f,indent=2,ensure_ascii=False)
		print(f"Saved {len(events)} events to {fn}")


def main():
	s=PoliticsCloudScraper()
	e=s.scrape_politics_events()
	if e:
		s.save_events(e)
		print(f"🎓 Successfully scraped {len(e)} Politics events!")
	else:
		print('💀 No Politics events found.')
