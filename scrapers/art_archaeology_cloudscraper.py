...
class ArtArchaeologyCloudScraper:
    def __init__(self):
        self.scraper = cloudscraper.create_scraper()
        self.base_url = "https://artandarchaeology.princeton.edu"
        self.events_url = f"{self.base_url}/events"
        
    def scrape_art_archaeology_events(self) -> List[Dict[str, Any]]:
        """Scrape events from the Art and Archaeology department"""
        events = []
        
        try:
            print(f"Scraping Art and Archaeology events from: {self.events_url}")
            response = self.scraper.get(self.events_url)
            response.raise_for_status()
            
            soup = BeautifulSoup(response.content, 'html.parser')
            
            # Look for event containers - Art and Archaeology typically uses structured event listings
            event_containers = soup.find_all(['div', 'article'], class_=re.compile(r'event|lecture|exhibition|talk|symposium', re.I))
            ...
            if not event_containers:
                ...
                event_containers = [div for div in event_containers if any(keyword in div.get_text().lower() 
                                                                       for keyword in ['lecture', 'exhibition', 'talk', 'event', 'art', 'archaeology', 'symposium'])]
            ...
            print(f"Found {len(events)} Art and Archaeology events")
            return events
            
        except Exception as e:
            print(f"Error scraping Art and Archaeology events: {e}")
            return []
    ...
    def _parse_page_for_events(self, soup) -> List[Dict[str, Any]]:
        ...
        for event_text in potential_events:
            ...
            if any(keyword in event_text.lower() for keyword in ['lecture', 'exhibition', 'talk', 'event', 'art', 'archaeology', 'symposium']):
                event = {
...
if __name__ == "__main__":
    scraper = ArtArchaeologyCloudScraper()
    events = scraper.scrape_art_archaeology_events()
    
    # Save to file
    with open('art_archaeology_events.json', 'w', encoding='utf-8') as f:
        json.dump(events, f, indent=2, ensure_ascii=False)
    
    print(f"Scraped {len(events)} events from Art and Archaeology")
