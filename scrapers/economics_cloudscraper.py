...
class EconomicsCloudScraper:
	...
	def scrape_economics_events(self):
		print("SCRAPING ECONOMICS EVENTS")
		print("="*60)
		try:
			with open('economics_cloudscraper_content.html', 'r', encoding='utf-8') as f:
				html = f.read()
				print(f"✅ Loaded saved HTML file, length: {len(html)}")
				return self.parse_events(html)
		except FileNotFoundError:
			print("Saved HTML file not found, fetching from URL...")
		url = 'https://economics.princeton.edu/events'
		try:
	...
	def parse_item(self, it):
		...
		if link:
			href = link['href']
			if href.startswith('/'):
				href = f"https://economics.princeton.edu{href}"
			elif not href.startswith('http'):
				href = f"https://economics.princeton.edu/{href}"
		return {
			'id': f"economics_{start_date}_{title[:20].replace(' ','_')}",
			'title': title,
			'description': title,
	...
	def extract_tags(self, title):
		...
		for tag in ['seminar','lecture','talk','colloquium','workshop','conference','symposium','economics','economic','macroeconomics','microeconomics','econometrics','development','labor','finance']:
	...
	def save_events(self, events):
		...
		fn='economics_cloudscraper_events.json'
		with open(fn,'w',encoding='utf-8') as f:
			json.dump({'metadata':{'total_events':len(events),'department':'Economics','scraped_at':datetime.now().isoformat()},'events':events},f,indent=2,ensure_ascii=False)
		print(f"Saved {len(events)} events to {fn}")


def main():
	s=EconomicsCloudScraper()
	e=s.scrape_economics_events()
	if e:
		s.save_events(e)
		print(f"🎓 Successfully scraped {len(e)} Economics events!")
	else:
