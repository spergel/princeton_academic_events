...
class ElectricalComputerEngineeringCloudScraper:
    def __init__(self):
        self.scraper = cloudscraper.create_scraper()
        self.base_url = "https://ece.princeton.edu"
        self.events_url = f"{self.base_url}/events"
        
    def scrape_electrical_computer_engineering_events(self) -> List[Dict[str, Any]]:
        """Scrape events from the Electrical and Computer Engineering department"""
        print(f"Scraping Electrical and Computer Engineering events from: {self.events_url}")
        ...
        try:
            ...
            print(f"Found {len(events)} Electrical and Computer Engineering events")
            return events
            
        except Exception as e:
            print(f"Error scraping Electrical and Computer Engineering events: {e}")
            return []
    ...
    def parse_event_item(self, item, index):
        ...
        try:
            ...
            # Create event object
            event = {
                'event_id': f"ece_{index + 1}_{self.sanitize_title(title)}",
                'title': title,
    ...
    def extract_description(self, item):
        ...
        speaker_elem = item.select_one('.field--name-field-ps-events-speaker .field--name-field-ps-event-speaker-name a')
        if speaker_elem:
    ...
    def extract_date_time(self, item):
        ...
        # The ECE page has multiple date fields - we want the one with the full date, not the badge
...
if __name__ == "__main__":
    scraper = ElectricalComputerEngineeringCloudScraper()
    events = scraper.scrape_electrical_computer_engineering_events()
    
    # Save to JSON for inspection
    with open('ece_events_test.json', 'w', encoding='utf-8') as f:
        json.dump(events, f, indent=2, ensure_ascii=False)
    
    print(f"\n💾 Saved {len(events)} events to ece_events_test.json")
