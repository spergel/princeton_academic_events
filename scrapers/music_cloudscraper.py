...
class MusicCloudScraper:
    def __init__(self):
        self.scraper = cloudscraper.create_scraper()
        self.base_url = "https://music.princeton.edu"
        self.events_url = f"{self.base_url}/events"
        
    def scrape_music_events(self) -> List[Dict[str, Any]]:
        """Scrape events from the Music department"""
        events = []
        
        try:
            print(f"Scraping Music events from: {self.events_url}")
            response = self.scraper.get(self.events_url)
            response.raise_for_status()
            
            soup = BeautifulSoup(response.content, 'html.parser')
            
            # Look for event containers - Music typically uses structured event listings
            event_containers = soup.find_all(['div', 'article'], class_=re.compile(r'event|concert|performance|recital', re.I))
            ...
            if not event_containers:
                ...
                event_containers = [div for div in event_containers if any(keyword in div.get_text().lower() 
                                                                       for keyword in ['concert', 'recital', 'performance', 'event', 'music', 'ensemble'])]
            ...
            print(f"Found {len(events)} Music events")
            return events
            
        except Exception as e:
            print(f"Error scraping Music events: {e}")
            return []
    ...
    def _parse_page_for_events(self, soup) -> List[Dict[str, Any]]:
        ...
        for event_text in potential_events:
            ...
            if any(keyword in event_text.lower() for keyword in ['concert', 'recital', 'performance', 'event', 'music', 'ensemble']):
                event = {
...
if __name__ == "__main__":
    scraper = MusicCloudScraper()
    events = scraper.scrape_music_events()
    
    # Save to file
    with open('music_events.json', 'w', encoding='utf-8') as f:
        json.dump(events, f, indent=2, ensure_ascii=False)
    
    print(f"Scraped {len(events)} events from Music")
