...
class OperationsResearchFinancialEngineeringCloudScraper:
    def __init__(self):
        self.scraper = cloudscraper.create_scraper()
        self.base_url = "https://orfe.princeton.edu"
        self.events_url = f"{self.base_url}/events"
        
    def scrape_operations_research_financial_engineering_events(self) -> List[Dict[str, Any]]:
        """Scrape events from the Operations Research and Financial Engineering department"""
        events = []
        
        try:
            print(f"Scraping Operations Research and Financial Engineering events from: {self.events_url}")
            response = self.scraper.get(self.events_url)
            response.raise_for_status()
            
            soup = BeautifulSoup(response.content, 'html.parser')
            
            # Look for event containers - ORFE typically uses structured event listings
            ...
            print(f"Found {len(events)} Operations Research and Financial Engineering events")
            return events
            
        except Exception as e:
            print(f"Error scraping Operations Research and Financial Engineering events: {e}")
            return []
...
if __name__ == "__main__":
    scraper = OperationsResearchFinancialEngineeringCloudScraper()
    events = scraper.scrape_operations_research_financial_engineering_events()
    
    # Save to file
    with open('operations_research_financial_engineering_events.json', 'w', encoding='utf-8') as f:
        json.dump(events, f, indent=2, ensure_ascii=False)
    
    print(f"Scraped {len(events)} events from Operations Research and Financial Engineering")
