...
class MolecularBiologyCloudScraper:
    def __init__(self):
        self.scraper = cloudscraper.create_scraper()
        self.base_url = "https://molbio.princeton.edu"
        self.events_url = f"{self.base_url}/events"
        
    def scrape_molecular_biology_events(self) -> List[Dict[str, Any]]:
        """Scrape events from the Molecular Biology department"""
        events = []
        
        try:
            print(f"Scraping Molecular Biology events from: {self.events_url}")
            response = self.scraper.get(self.events_url)
            response.raise_for_status()
            
            soup = BeautifulSoup(response.content, 'html.parser')
            
            # Look for event containers - Molecular Biology typically uses structured event listings
            event_containers = soup.find_all(['div', 'article'], class_=re.compile(r'event|seminar|talk', re.I))
            
            if not event_containers:
                # Try alternative selectors
                event_containers = soup.find_all(['div', 'article'], class_=re.compile(r'item|entry|post', re.I))
            ...
            if not event_containers:
                ...
                event_containers = [div for div in event_containers if any(keyword in div.get_text().lower() 
                                                                        for keyword in ['seminar', 'talk', 'lecture', 'event', 'presentation'])]
            ...
            print(f"Found {len(events)} Molecular Biology events")
            return events
            
        except Exception as e:
            print(f"Error scraping Molecular Biology events: {e}")
            return []
    ...
    def _parse_page_for_events(self, soup) -> List[Dict[str, Any]]:
        ...
        for event_text in potential_events:
            ...
            if any(keyword in event_text.lower() for keyword in ['seminar', 'talk', 'lecture', 'presentation', 'event']):
                event = {
...
if __name__ == "__main__":
    scraper = MolecularBiologyCloudScraper()
    events = scraper.scrape_molecular_biology_events()
    
    # Save to file
    with open('molecular_biology_events.json', 'w', encoding='utf-8') as f:
        json.dump(events, f, indent=2, ensure_ascii=False)
    
    print(f"Scraped {len(events)} events from Molecular Biology")
