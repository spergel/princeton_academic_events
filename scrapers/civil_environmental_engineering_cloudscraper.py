...
class CivilEnvironmentalEngineeringCloudScraper:
    def __init__(self):
        self.scraper = cloudscraper.create_scraper()
        self.base_url = "https://cee.princeton.edu"
        self.events_url = f"{self.base_url}/events"
        
    def scrape_civil_environmental_engineering_events(self) -> List[Dict[str, Any]]:
        """Scrape events from the Civil and Environmental Engineering department"""
        print(f"Scraping Civil and Environmental Engineering events from: {self.events_url}")
        ...
        try:
            ...
            print(f"Found {len(events)} Civil and Environmental Engineering events")
            return events
            
        except Exception as e:
            print(f"Error scraping Civil and Environmental Engineering events: {e}")
            return []
    ...
    def parse_event_item(self, item, index):
        ...
        try:
            ...
            # Create event object
            event = {
                'event_id': f"cee_{index + 1}_{self.sanitize_title(title)}",
                'title': title,
...
if __name__ == "__main__":
    scraper = CivilEnvironmentalEngineeringCloudScraper()
    events = scraper.scrape_civil_environmental_engineering_events()
    
    # Save to JSON for inspection
    with open('cee_events_test.json', 'w', encoding='utf-8') as f:
        json.dump(events, f, indent=2, ensure_ascii=False)
    
    print(f"\n💾 Saved {len(events)} events to cee_events_test.json")
